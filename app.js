...
function statCard(icon,value,label,color,sub=""){const isText=isNaN(parseFloat(String(value)))&&String(value).replace(/[%]/g,"").trim().length>2;const valFs=isText?"19px":"26px";return `<div class="card-hover" style="background:${T.surface};border:1px solid ${T.border};border-radius:16px;padding:22px 20px 22px 20px;position:relative;overflow:hidden;box-shadow:${T.shadow};min-width:0;"><div style="position:absolute;top:14px;right:14px;width:42px;height:42px;border-radius:12px;background:${color}18;display:flex;align-items:center;justify-content:center;font-size:18px;overflow:hidden;flex-shrink:0;line-height:1">${icon}</div><div style="font-size:${valFs};font-weight:800;color:${color};font-family:'Space Grotesk',sans-serif;line-height:1.1;padding-right:50px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%">${esc(String(value))}</div><div style="font-size:13px;color:${T.muted};margin-top:6px;font-weight:600">${esc(label)}</div>${sub?`<div style="font-size:11px;color:${color};margin-top:3px;font-weight:600;opacity:.8">${esc(sub)}</div>`:""}</div>`;}
...
function renderStudentDash(s){
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:14px;margin-bottom:22px">
    ${statCard("✅",`${attPct}%`,"Attendance",attPct>=75?T.green:T.red,`${pd} days`)}${statCard("📈",avg||"-","Avg Score",gradeColor(avg||0),gradeLabel(avg||0))}${statCard("💳",s.feeStatus,"Fee",s.feeStatus==="paid"?T.green:s.feeStatus==="overdue"?T.red:T.yellow)}${statCard("📎",pendingA.length,"Pending Tasks",T.blue,"Assignments")}
  </div>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:18px">
